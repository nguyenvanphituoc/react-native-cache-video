...
| [readiness-observation](scopes/readiness-observation.json) | LAYER_CAKE | TASK-007, 011, 013 (3) | 7 globs / 12 files — manager/hook + index.tsx + both example apps | ProxyCacheManager.ts, useProxyCacheProvider.tsx | jest (issue-6\|readiness) + typecheck; simulator visual → TBD(manual) | ✅ clean |
...
| [regression-net](scopes/regression-net.json) | CHOWDER | TASK-001, 015 (2) | 6 globs / 3 files — mocks + harness + integration test + jest config | — | full `yarn test` + typecheck | ✅ clean |

**Coverage:** 15/15 board tasks mapped, each to exactly one scope.
...
U1 `single-video-player` → readiness-observation; U3 `console-fallback-warning` →
reasoned-fallback. server-start-truth, verified-cache-pipeline, and regression-net own no
UI elements (code-affordance/iceberg scopes).
