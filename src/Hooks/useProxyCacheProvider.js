 ...
 * "
 */
export const CacheManagerContext = createContext(new CacheManager('react-native-cache-video', __DEV__));
CacheManagerContext.displayName = Symbol('CacheManagerContext').toString();
export const lfuPolicy = new LFUPolicy();
export const CacheManagerProvider = ({ capacity, cachePolicy = lfuPolicy, children, }) => {
    const cacheManager = useRef(new CacheManager('react-native-cache-video', __DEV__));
    //
    ...
    return (React.createElement(CacheManagerContext.Provider, { value: cacheManager.current }, children));
};
