import { HLS_CONTENT_TYPE, HLS_VIDEO_TYPE, KEY_PREFIX, SIGNAL_NOT_DOWNLOAD_ACTION,
// VIDEO_EXTENSIONS,
 } from './Utils/constants';
import { FileBucket, SimpleFileProvider } from './Provider/FileProvider';
import { cacheKey, getCacheKey, getOriginURL, portGenerate, reverseProxyPlaylist, reverseProxyURL, } from './Utils/util';
import { SimpleSessionProvider, } from './Provider/SessionProvider';
...
export class CacheManager {
    ...
    _memoryCache;
    //
    constructor(serverName, devMode, _sessionTask = new SimpleSessionProvider(), _storage = new SimpleFileProvider()) {
        //
    ...
    putCachedFile(forKey, folder = this.cacheFolder) {
        //
        const { originURL, cacheKey: cacheKeyStr } = getCacheKey(forKey, folder, KEY_PREFIX);
    ...
    async getCachedFileAsync(url, folder = this.cacheFolder) {
        ...
        const { originURL, cacheKey: cacheKeyStr } = getCacheKey(url, folder, KEY_PREFIX);
    ...
    enableMemoryCache(capacity, cachePolicy) {
        if (!this._memoryCache) {
            this._memoryCache = new MemoryCacheProvider(capacity, cachePolicy);
    ...
    async onCachingPlaylistSource(forUrl, data, folder) {
        const { originURL, cacheKey: cacheKeyStr } = getCacheKey(forUrl, folder, KEY_PREFIX);
    ...
    reverseProxyURL(forUrl) {
        if (!forUrl.startsWith('http') || !this._bridgeServer?.port) {
            console.warn('reverseProxyURL: invalid url or port. Should check if bridge server is running and url is CDN url start with http');
            return forUrl;
        }
    ...
    async addPlaylistHandler(forUrl, filePath, reverseRes) {
        try {
            const port = this._bridgeServer.port;
            //
            if (this._memoryCache?.has(forUrl)) {
                // make playlist
                let playlistStr = reverseProxyPlaylist(this.getCachedFile(forUrl), forUrl, port);
            ...
            const { data, error, ...response } = await this._sessionTask.dataTask(forUrl, {});
            if (error) {
                return reverseRes.send(500, 'text/plain', 'Cannot get data from origin server');
            }
            this.syncMemoryCache(forUrl, data);
            reverseRes.send(response.respInfo.status, response.respInfo.headers['Content-Type'], reverseProxyPlaylist(data, forUrl, port));
    ...
    async addSegmentHandler(forUrl, filePath, reverseRes) {
        try {
            if (this._memoryCache?.has(forUrl)) {
                // TODO:
            }
            ...
            // this.syncMemoryCache(forUrl, data);
            reverseRes.send(response.respInfo.status, response.respInfo.headers['Content-Type'], data);
            this._storage.write(filePath, data);
        }
        catch (error) {
            throw error;
